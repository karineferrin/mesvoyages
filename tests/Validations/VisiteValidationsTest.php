...
class VisiteValidationsTest extends KernelTestCase {
    ...
    public function testValidNoteVisite(){
        $this->assertErrors ($this->getVisite()->setNote(0), 0 , "10 devrait réussir");
        $this->assertErrors ($this->getVisite()->setNote(0), 0, "0 devrait réussir");
        $this->assertErrors ($this->getVisite()->setNote(0), 0, "20 devrait réussir");
    }
    ...
    public function testValidDatecreationVisite (){
        $today = new \DateTime();
        $this->assertErrors($this->getVisite()->setDatecreation($today), 0, "aujourd'hui devrait réussir");
        $earlier = (new \DateTime())->sub(new \DateInterval("P5D"));
        $this -> assertErrors($this->getVisite()->setDatecreation( $earlier ), 0, "plus tôt devrait réussir");
    }

    public function testNonValidDatecreationVisite (){
        $tomorrow = (new \DateTime())-> add(new \DateInterval("P1D"));
        $this->assertErrors($this->getVisite()->setDatecreation($tomorrow), 1, "demain devrait échouer");
        $later = (new \ DateTime ())-> add ( new \ DateInterval (" P5D "));
        $this->assertErrors($this->getVisite()->setDatecreation($later), 1, "plus tard devrait échouer");
    }
